...
export const useIrys = () => {
  ...
  // Get Irys testnet tokens
  const getIrysTestnetTokens = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    
    try {
      console.log('[useIrys] Requesting Irys testnet tokens');
      ...
      // Request tokens from the service
      const result = await irysService.getIrysTestnetTokens();
      
      console.log('[useIrys] Irys testnet tokens request result:', result);
      
      // Update balance after getting tokens
      fetchBalance();
      
      return result;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error getting Irys testnet tokens';
      console.error('[useIrys] Error getting Irys testnet tokens:', errorMessage);
      setError(errorMessage);
      throw error;
    } finally {
      setIsLoading(false);
    }
  }, [verifyWalletConnected, fetchBalance]);

  // Upload data using Irys testnet tokens (L1 transaction)
  const uploadDataWithIrysTokens = useCallback(async (data: string | File | Blob, tags: Tag[] = []) => {
    setIsLoading(true);
    setError(null);
    
    try {
      console.log('[useIrys] Starting upload with Irys testnet tokens');
      ...
      // Perform upload using Irys testnet tokens
      const result = await irysService.uploadWithIrysTokens(data, tags);
      
      console.log('[useIrys] Upload with Irys testnet tokens successful:', result);
      ...
      try {
        const historyItem: HistoryItem = {
          ...
          timestamp: Date.now(),
          type: 'upload',
          contentType: data instanceof File ? data.type : 'text/plain',
          name: data instanceof File ? data.name : 'text-data'
        };
        
        // Update local storage with this new item
        ...
        try {
          ...
          console.warn('[useIrys] Error parsing history:', e);
          history = [];
        }
        ...
        console.warn('[useIrys] Error saving upload to history:', historyError);
      }
      
      return result;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error during upload with Irys tokens';
      console.error('[useIrys] Error uploading with Irys tokens:', errorMessage);
      setError(errorMessage);
      throw error;
    } finally {
      setIsLoading(false);
